                ...
                @if($post->date)
                    <aside class="post__header__meta">
                        @if($post->original_publication_url && $post->original_publication_name)
                            Originally published on <time datetime="{{ $post->date->format('Y-m-d') }}">{{ $post->date->format('F jS, Y') }}</time> by Gunharth Randolf
                            on <a href="{{ $post->original_publication_url }}" class="post__header__meta__link">{{ $post->original_publication_name }}</a>
                        @else
                            Published on <time datetime="{{ $post->date->format('Y-m-d') }}">{{ $post->date->format('F jS, Y') }}</time> by Gunharth Randolf
                        @endif
                        @if($post->subtitle)
                            — {{ $post->subtitle }}
                        @endif
        ...
        <footer class="post__footer">
            <div class="container container--narrow">
                <p class="post__footer__p">
                    © {{ carbon()->format('Y') }}
                    <a class="post__footer__link -subtle" href="{{ route('about') }}">Gunharth Randolf</a>
                </p>
                <p class="post__footer__p">
